...
Vector2 Input::getMouseOffset() const
{
	return _currentMousePosition - _lastMousePosition;
}

Vector2 Input::getLastMousePosition() const
{
	return _lastMousePosition;
}

Vector2 Input::getCurrentMousePosition() const
{
...
bool Input::isPressedOrHeld(Key key) const
{
	const auto state = getStateOfKey(key);
	return state == State::Pressed or state == State::Held;
}

Input::State Input::getStateOfKey(Key key) const
{
...
{
	return isKeyBelongsToMouse(key) ?
		   Window::getInstance()->getMouseKeyState(key) :
		   Window::getInstance()->getKeyboardKeyState(key);
}
...
void Input::refreshMousePosition()
{
	...
	const auto mousePosition = Window::getInstance()->getMousePosition();
